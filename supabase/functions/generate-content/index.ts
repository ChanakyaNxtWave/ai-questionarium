...
const topicsPrompt = `I want you to act as a technical instructional designer.
...
const questionsPrompt = `**Objective:** You are an technical instructional Designer tasked with developing a set of multiple-choice questions covering all the learning outcomes listed based on the session summary and reading material.

**Input:**
{session_topics_and_learning_outcome}
...
serve(async (req) => {
  ...
  try {
    ...
    let systemPrompt = type === 'topics' ? topicsPrompt : questionsPrompt;
    let userContent = content;
    
    if (type === 'topics') {
      userContent = content.replace('{details}', content);
    } else if (type === 'questions') {
      userContent = content.replace('{session_topics_and_learning_outcome}', content);
    }
